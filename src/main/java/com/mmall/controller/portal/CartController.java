...
public class CartController {
	...
	@ResponseBody
	public ServerResponse remove(HttpSession session,String productIds,Integer count){
		User user = (User) session.getAttribute(Const.CURRENT_USER);
		if(user == null){
			return ServerResponse.createByErrorCodeMessage(ResponseCode.NEED_LOGIN.getCode(), "需要登陆才能使用购物车.");
		}
	...
	@ResponseBody
	public ServerResponse selectOneProduct(HttpSession session,Integer productId){
		User user = (User) session.getAttribute(Const.CURRENT_USER);
		if(user == null){
			return ServerResponse.createByErrorCodeMessage(ResponseCode.NEED_LOGIN.getCode(), "需要登陆才能使用购物车.");
		}
